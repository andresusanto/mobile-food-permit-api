...
test("different limit size", async () => {
  const searcher = indexPermitsFuzzy(testData);
  for (const limit of [1, 2, 3, 4, 5]) {
    const res = searcher("APPROVED", limit);
    expect(res.length).toEqual(limit);
  }
});

test("specific search query", async () => {
  const searcher = indexPermitsFuzzy(testData);
  for (const { query, result } of [
    { query: "California", result: 1 },
    { query: "LINCOLN", result: 1 },
    { query: "El Alambre", result: 2 },
    { query: "Drinks", result: 4 },
  ]) {
    const res = searcher(query, 5);
